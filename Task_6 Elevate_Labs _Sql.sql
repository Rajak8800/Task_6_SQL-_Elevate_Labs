...
-- Total_Sales 
 select round(sum(sales ),2)as Total_Sales from Orders_data ;
...
-- Monthly sales 
select date_format(order_date, '%b') as monthly , round(sum(sales),2) as Total_Sales 
from Orders_data 
group by monthly 
order by Total_Sales desc 
limit 5;
...
--  which month had higest Return Rate 
...
FROM 
    orders_with_returns
GROUP BY 
    DATE_FORMAT(order_date, '%b'), MONTH(order_date)
ORDER BY 
    MONTH(order_date);
...
-- Avg Delivery Time
SELECT 
    concat(ROUND(AVG(DATEDIFF(ship_date, order_date)), 2), ' ' 'Days') AS Avg_Shipping_Time_Days
FROM 
    Orders_data;
 ...
 --  What percentage of orders are returned per category?
    ...
    COUNT(o.order_id) AS Total_Orders,
    COUNT(r.order_id) AS Total_Returns,
    ROUND(COUNT(r.order_id)/COUNT(o.order_id) * 100, 2) AS Return_Percentage
FROM 
    orders_data o
LEFT JOIN 
    returns_data r ON o.order_id = r.order_id
...
-- Which customers returned the most items?   
SELECT 
    o.order_id,
    COUNT(r.order_id) AS Total_Returns
FROM 
